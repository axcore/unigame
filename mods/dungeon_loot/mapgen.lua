...
--      end
        -- @@@ new
        if minetest.get_node(chestpos).name == "air" and
                minetest.registered_nodes["unilib:container_chest_ordinary"] ~= nil then

            -- make it face inwards to the room
            local facedir = minetest.dir_to_facedir(vector.multiply(wall.facing, -1))
            minetest.add_node(chestpos, {name = "unilib:container_chest_ordinary", param2 = facedir})
            populate_chest(chestpos, PcgRandom(noise3d_integer(noise, chestpos)), room.type)
